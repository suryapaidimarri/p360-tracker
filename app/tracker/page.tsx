...
export default function TrackerPage() {
  ...
  const loadData = useCallback(async () => {
    const [{ data: p }, { data: t }, { data: a }] = await Promise.all([
      ...
      supabase.from('tracker_ai_ideas').select('*, tracker_ai_idea_projects(project_id)').order('created_at', { ascending: false }),
    ]);
    if (p) { setProjects(p); setView(v => v || (p[0]?.id ?? null)); }
    if (t) setTasks(t);
    if (a) setAiIdeas(a.map((i: any) => ({ ...i, project_ids: (i.tracker_ai_idea_projects ?? []).map((x: any) => x.project_id) })));
  }, []);
  ...
  async function createProject() {
    ...
    const { data } = await supabase.from('tracker_projects').insert({ name: fProjName.trim(), description: fProjDesc, color: fProjColor }).select().single();
  ...
  async function saveAIIdea() {
    ...
    const { data: idea } = await supabase.from('tracker_ai_ideas').insert({ title: fAiTitle.trim(), description: fAiDesc || 'AI integration idea' }).select().single();
    if (idea && fAiProjs.length > 0) {
      await supabase.from('tracker_ai_idea_projects').insert(fAiProjs.map(pid => ({ idea_id: idea.id, project_id: pid })));
    }
  ...
  if (loading) return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#FAFAFA', fontFamily: 'sans-serif', color: '#6B6B6B', fontSize: 14 }}>Loading…</div>;
            ...
            {projects.map(p => {
              ...
              return (
                <div key={p.id} onClick={() => setView(p.id)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 8px', border: `1px solid ${on ? '#111' : 'transparent'}`, background: on ? '#111' : 'transparent', marginBottom: 2, cursor: 'pointer' }}>
                  ...
                  <span style={{ fontFamily: "'Barlow',sans-serif", fontSize: 9, fontWeight: 600, background: on ? 'rgba(255,255,255,.15)' : '#F0F0F0', color: on ? '#fff' : '#6B6B6B', padding: '2px 6px' }}>{dn}/{tot}</span>
                </div>
              );
            })}
        ...
        <div style={S.main}>
          {!view && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 360, textAlign: 'center' }}>
              <div style={{ fontSize: 26, fontWeight: 400, marginBottom: 8 }}>Welcome to P360 Tracker</div>
          ...
          {view === 'ai' && (
            <>
              <div style={S.ph}>
                ...
                <button style={btn('accent')} onClick={() => { setFAiTitle(''); setFAiDesc(''); setFAiProjs([]); setAiModal(true); }}>+ Add AI Idea</button>
              </div>
              <div style={{ border: '1px solid #E6E6E6', background: '#fff' }}>
                ...
                {aiIdeas.length === 0 && <div style={{ padding: 48, textAlign: 'center', color: '#6B6B6B', fontSize: 13 }}>No AI ideas yet. Add one above.</div>}
                    ...
                    return (
                      <div key={idea.id} style={{ padding: '16px 18px', borderRight: '1px solid #E6E6E6', borderBottom: '1px solid #E6E6E6', position: 'relative' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                          {linked.length > 0 ? linked.map(p => <span key={p.id} style={{ fontFamily: "'Barlow',sans-serif", fontSize: 8, fontWeight: 600, padding: '2px 6px', border: `1px solid ${p.color}40`, background: `${p.color}15`, color: p.color }}>{p.name}</span>) : <span style={{ fontFamily: "'Barlow',sans-serif", fontSize: 8, padding: '2px 6px', background: '#F0F0F0', color: '#6B6B6B' }}>No project linked</span>}
                        </div>
                        <button onClick={() => deleteAIIdea(idea.id)} style={{ ...btn('danger', true), position: 'absolute', top: 10, right: 10, padding: '3px 6px' }}>✕</button>
                      </div>
                    );
                  })}
                </div>
              ...
              <>
                <div style={S.ph}>
                  <div>
                    ...
                    <h1 style={S.phTitle}><span style={{ display: 'inline-block', width: 8, height: 32, background: proj.color, flexShrink: 0 }} />{proj.name}</h1>
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 22, flexWrap: 'wrap' }}>
                  {[{ b:'#20BB71',n:done,l:'Done'},{b:'#20BB71',n:pts.filter(t=>t.column_name==='inprogress').length,l:'In Progress'},{b:'#F9B62A',n:pts.filter(t=>t.column_name==='next').length,l:'Up Next'},{b:'#48B5EA',n:pts.filter(t=>t.column_name==='planning').length,l:'Planning'}].map(s => (
                  ...
                  <div style={{ background: '#fff', border: '1px solid #E6E6E6', padding: '12px 16px', minWidth: 160 }}>
                    ...
                    <div style={{ height: 3, background: '#F0F0F0', marginBottom: 5 }}><div style={{ height: '100%', background: '#20BB71', width: `${pct}%` }} /></div>
                    <div style={{ fontFamily: "'Barlow',sans-serif", fontSize: 9, color: '#6B6B6B' }}>{pct}% · {done}/{total} done</div>
                  </div>
                </div>
                    ...
                    return (
                      <div key={col.key} style={{ background: '#fff', border: '1px solid #E6E6E6' }}>
                        <div style={{ padding: '11px 14px 9px', borderBottom: '1px solid #E6E6E6', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          ...
                          <span style={{ fontFamily: "'Barlow',sans-serif", fontSize: 9, fontWeight: 500, color: '#6B6B6B', background: '#F0F0F0', padding: '1px 6px' }}>{colTasks.length}</span>
                        </div>
                        <div style={{ padding: 10, minHeight: 200, display: 'flex', flexDirection: 'column', gap: 7 }}>
                          {colTasks.length === 0 && <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 120, border: '1px dashed #E6E6E6' }}><span style={{ fontFamily: "'Barlow',sans-serif", fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#E6E6E6' }}>Empty</span></div>}
                          {colTasks.map(task => {
                            const dl = dlInfo(task.deadline);
                            const tv: ColKey | 'crit' | 'high' = task.priority === 'Critical' ? 'crit' : task.priority === 'High' ? 'high' : col.key;
                            const tl = task.priority === 'Critical' ? 'Critical' : task.priority === 'High' ? 'High' : col.key === 'done' ? 'Done' : col.key === 'inprogress' ? 'Active' : col.key === 'next' ? 'Up next' : 'Planned';
                            return (
                              <div key={task.id} style={{ border: '1px solid #E6E6E6', background: '#fff', padding: '10px 12px' }}>
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 7, alignItems: 'center' }}>
                                  <span style={tag(tv)}><span style={{ width: 3, height: 3, borderRadius: '50%', background: 'currentColor', opacity: 0.7 }} />{tl}</span>
                                  {dl && <span style={{ fontFamily: "'Barlow',sans-serif", fontSize: 8, fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', color: dl.c }}>{dl.t}</span>}
                                </div>
                                <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                                  <button style={{ ...btn('ghost', true), padding: '3px 7px', fontSize: 11 }} onClick={() => openEditTask(task)}>✎</button>
                                  {col.moveTo && <button style={{ background: '#111', border: '1px solid #111', color: '#fff', padding: '4px 7px', cursor: 'pointer', fontSize: 10, fontFamily: "'DM Sans',sans-serif", whiteSpace: 'nowrap' }} onClick={() => moveTask(task.id, col.moveTo!)}>{col.moveLabel}</button>}
                                  <button style={{ ...btn('danger', true), padding: '3px 7px', fontSize: 11 }} onClick={() => deleteTask(task.id)}>✕</button>
                                </div>
                              </div>
                            );
                          })}
      ...
      {projModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }} onClick={e => { if (e.target === e.currentTarget) setProjModal(false); }}>
          <div style={{ background: '#fff', border: '1px solid #E6E6E6', padding: 28, width: 440, maxWidth: '90vw', boxShadow: '0 6px 18px rgba(0,0,0,.08)' }}>
            <div style={{ fontSize: 20, fontWeight: 400, marginBottom: 3 }}>New Project</div>
            ...
            <MField label="Color">
              <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginTop: 4 }}>
                {PROJECT_COLORS.map(c => <div key={c} onClick={() => setFProjColor(c)} style={{ width: 26, height: 26, background: c, cursor: 'pointer', border: `2px solid ${fProjColor === c ? '#111' : 'transparent'}` }} />)}
              </div>
            </MField>
      ...
      {taskModal.open && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }} onClick={e => { if (e.target === e.currentTarget) setTaskModal({ open: false }); }}>
          <div style={{ background: '#fff', border: '1px solid #E6E6E6', padding: 28, width: 480, maxWidth: '90vw', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 6px 18px rgba(0,0,0,.08)' }}>
            <div style={{ fontSize: 20, fontWeight: 400, marginBottom: 3 }}>{taskModal.editing ? 'Edit Task' : 'Add Task'}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <MField label="Project"><select value={fProject} onChange={e => setFProject(e.target.value)} style={iStyle}>{projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></MField>
              <MField label="Column"><select value={fCol} onChange={e => setFCol(e.target.value as ColKey)} style={iStyle}>{COLS.map(c => <option key={c.key} value={c.key}>{c.label}</option>)}</select></MField>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <MField label="Deadline"><input type="date" value={fDeadline} onChange={e => setFDeadline(e.target.value)} style={iStyle} /></MField>
              <MField label="Priority"><select value={fPriority} onChange={e => setFPriority(e.target.value)} style={iStyle}><option value="">— None —</option><option value="Critical">Critical</option><option value="High">High</option></select></MField>
            </div>
      ...
      {aiModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }} onClick={e => { if (e.target === e.currentTarget) setAiModal(false); }}>
          <div style={{ background: '#fff', border: '1px solid #E6E6E6', padding: 28, width: 460, maxWidth: '90vw', boxShadow: '0 6px 18px rgba(0,0,0,.08)' }}>
            <div style={{ fontSize: 20, fontWeight: 400, marginBottom: 3 }}>Add AI Idea</div>
            ...
            <MField label="Link to Projects (optional)">
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 6 }}>
                {projects.map(p => <label key={p.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: 12 }}><input type="checkbox" checked={fAiProjs.includes(p.id)} onChange={e => setFAiProjs(prev => e.target.checked ? [...prev, p.id] : prev.filter(x => x !== p.id))} /><span style={{ width: 8, height: 8, borderRadius: '50%', background: p.color, display: 'inline-block' }} />{p.name}</label>)}
              </div>
            </MField>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 18, paddingTop: 16, borderTop: '1px solid #E6E6E6' }}>
